...
exports.RedisHealthIndicator = exports.PrismaHealthIndicator = void 0;
...
let PrismaHealthIndicator = class PrismaHealthIndicator extends terminus_1.HealthIndicator {
    ...
    async pingCheck(key = 'prisma') {
        try {
            await this.prisma.$queryRaw `SELECT 1`;
            return this.getStatus(key, true);
        }
        catch (err) {
            throw new terminus_1.HealthCheckError('Prisma ping failed', this.getStatus(key, false, { message: err.message }));
        }
    }
};
...
let RedisHealthIndicator = class RedisHealthIndicator extends terminus_1.HealthIndicator {
    ...
    async pingCheck(key = 'redis') {
        try {
            ...
            throw new terminus_1.HealthCheckError('Redis ping failed', this.getStatus(key, false, { message: err.message }));
        }
    }
};
